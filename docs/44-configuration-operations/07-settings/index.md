...
Unlike [configuration](/docs/configuration-operations/configuration), which is loaded once at startup, settings persist across restarts
and can be modified dynamically.

Settings are a general-purpose store for any value your application needs at runtime.
Arrower itself uses settings for [logging](/docs/configuration-operations/observability/logging#settings), auth, and the admin context.
...
|-----|------|-------------|
| `alog.SettingLogLevel` | `string` | Change the log level at runtime across all loggers. See [Logging](/docs/configuration-operations/observability/logging#settings) |
